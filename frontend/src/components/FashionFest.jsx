...
export default function FashionFestPage() {
    ...
    try {
      if (isEditing) {
        const response = await axios.put(
          `${API_URL}/${selectedFest._id}`,
          formData,
          { headers }
        );
        setFests(
          fests.map((fest) =>
            fest._id === selectedFest._id ? response.data : fest
          )
        );
      } else {
      ...
      const message =
        ...
        `Failed to ${isEditing ? "update" : "create"} fest. Please try again.`;
      setError(message);
  ...
  const handleUpvote = async (festId) => {
    ...
    try {
      ...
      setFests(
        fests.map((fest) => (fest._id === festId ? response.data : fest))
      );
    ...
    <div className="bg-light min-vh-100">
      ...
      >
        <div className="container text-center">
          ...
          <p className="lead">
            Discover and participate in fashion events near you
          </p>
        </div>
      </div>
                ...
                <div className="card-body d-flex flex-column">
                  ...
                  >
                    ...
                    <p className="card-text text-secondary">
                      {fest.description.slice(0, 80)}...
                    </p>
                  </div>
                  <div className="d-flex justify-content-between align-items-center mt-3">
                    <button
                      className="btn btn-outline-primary btn-sm d-flex align-items-center"
                      onClick={() => handleUpvote(fest._id)}
                    >
      ...
      <button
        className="btn btn-danger rounded-circle position-fixed bottom-0 end-0 m-4 p-0 d-flex align-items-center justify-content-center"
        onClick={() => {
          setIsEditing(false);
          setShowForm(true);
        }}
            ...
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">
                  {isEditing ? "Edit Fashion Fest" : "List Your Fashion Fest"}
                </h5>
                <button
                  type="button"
                  className="btn-close"
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="modal-footer">
                    ...
                    <button type="submit" className="btn btn-danger">
                      {isEditing ? "Update Fest" : "List Fest"}
                    </button>
                  </div>
                </form>
              </div>
        ...
        <div className="modal fade show d-block" tabIndex="-1">
          <div className="modal-dialog modal-lg modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>
                  <strong>Location:</strong> {selectedFest.location},{" "}
                  {selectedFest.city}
                </p>
                <p>
                  <strong>Dates:</strong> {formatDate(selectedFest.startDate)}{" "}
                  to {formatDate(selectedFest.endDate)}
                </p>
                <p>
                  <strong>GST:</strong> {selectedFest.gstNumber}
                </p>
                ...
                <div className="d-flex align-items-center">
                  <button
                    ...
                    onClick={() => handleUpvote(selectedFest._id)}
                  >
                    <ThumbsUp size={16} className="me-2" />{" "}
                    {selectedFest.upvotes.length} Interested
                  </button>
                  <p className="text-muted mb-0">
                    Organized by: {selectedFest.createdBy?.name || "Unknown"}
                  </p>
                </div>
              </div>
              <div className="modal-footer justify-content-between">
                <div>
                  <button
                    ...
                    onClick={() => handleEdit(selectedFest)}
                  >
